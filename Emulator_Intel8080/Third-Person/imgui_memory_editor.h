...
struct MemoryEditor : public IThemeLoadable
{
    ...
    MemoryEditor() : IThemeLoadable(u8"Просмотр памяти")
    {
        ...
        InitListWord({
            u8"Пустые ячейки (текст)",
            u8"Посещённая ячейка",
            u8"Текущая ячейка (PC)",
            u8"Ячейка останова",
            u8"Указатель на память (HL)"
            });
    ...
    std::vector<NamedColor> GetDefaultLightColors() override {
        return {
            {u8"Пустые ячейки (текст)",     ImColor(118,97,76,255)},
    ...
    void DrawContents(I8080 * processor, Widget_MnemocodeViewer* widget_MnemocodeViewer, const vector<OpcodeAdressed>& ca, const vector<ImVec4>& colorsCommand)
    {
        ...
        void* mem_data_void = processor->GetMemory();
        size_t base_display_addr = 0x0000;
    ...
    void DrawWindow(const char* title,unsigned int cursor, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
        CalcSizes(s, mem_size, base_display_addr);
        ImGui::SetNextWindowSize(ImVec2(s.WindowWidth, s.WindowWidth * 0.60f), ImGuiCond_FirstUseEver);
        ...
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar))
        {
            if (ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && ImGui::IsMouseReleased(ImGuiMouseButton_Right))
                ImGui::OpenPopup("context");
            DrawContents(mem_data, cursor, mem_size, base_display_addr);
            if (ContentsWidthChanged)
            {
                CalcSizes(s, mem_size, base_display_addr);
                ImGui::SetWindowSize(ImVec2(s.WindowWidth, ImGui::GetWindowSize().y));
    ...
    // Memory Editor contents only
    void DrawContents(void* mem_data_void,unsigned int cursor, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        if (Cols < 1)
            Cols = 1;
            ...
            for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
            {
                ...
                for (int n = 0; n < Cols && addr < mem_size; n++, addr++)
                {
                    ...
                    else
                    {
                        ...
                        if (cursor == addr)
                        {
                            draw_list->AddRectFilled(pos, ImVec2(pos.x + highlight_width, pos.y + s.LineHeight), ImGui::GetColorU32(ImVec4(0.1f, 0.3f, 0.1f, 1.0f)));
                        }
