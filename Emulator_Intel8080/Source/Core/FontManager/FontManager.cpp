...
void FontManager::DrawSetting() {
	...
	if (ImGui::BeginCombo(u8"Шрифт", Name_Font_Selected.c_str(), ImGuiComboFlags_WidthFitPreview)) {
		...
		if (ButtonCenteredOnLine(u8"Добавить свой шрифт", 0.5f)) {
			std::wstring outPath = Open_FileDialog(L"Выбери файл шрифта", L"Файл шрифта (*.ttf;*.otf)\0*.ttf;*.otf\0");
			if (!outPath.empty()) {
	...
	ImGui::SameLine();
	MyHelpMarker(u8"Выбор типа шрифта");
	...
	ImGui::SameLine();
	MyHelpMarker(u8"Единица измерения - пиксель.");



}

nlohmann::json FontManager::SaveSetting() {

	nlohmann::json result;

	result["scale"] = Current_Scale;
	result["selected"] = Name_Font_Selected;

	for (size_t i = 0; i < List_Fonts.size(); i++) {
		result["paths"][i] = List_Fonts[i].Path;
	}

	return result;
}

void FontManager::LoadSetting(const nlohmann::json& Data) {
	...
	if (Data.contains("scale"))
		SetSize(Data["scale"].get<int>());

	if (Data.contains("selected"))
		SetFont(Data["selected"].get<std::string>());
