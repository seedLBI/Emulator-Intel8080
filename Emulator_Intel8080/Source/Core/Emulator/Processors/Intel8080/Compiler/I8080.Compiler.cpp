		...
		for (int i_element = 0; i_element < splited_code[Line].size(); ++i_element){
			...
			if (isDerektiv_or_localMarker) {
				...
				if (command == "adr") {
					...
					i_element++;
					CurrentAdress = (int)value.first;
					adressed_code[Line] = (int)value.first;
					...
					Consts[name_const] = (uint16_t)value_const.first;
					...
					if (countCommandsAfterLastMarker == 0 && Markers.empty() == false){
						...
						const int count_vars = (int)splited_code[Line].size() - (i_element + 1);
			...
			if (pdi.flag_have_dot && pdi.flag_have_2dot) {
				...
				if (command == "adr") {
					...
					while (splited_code[Line].size() > i_element) {
						...
						ca.opcode = (unsigned int)value.first;
