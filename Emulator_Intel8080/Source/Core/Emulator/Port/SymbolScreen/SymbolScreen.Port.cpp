#include "SymbolScreen.Port.h"



Port_SymbolScreen::Port_SymbolScreen() :Port(0x07) {
...
void Port_SymbolScreen::SetInput(const uint8_t& value) {
...
uint8_t Port_SymbolScreen::GetOutput() {
	count_input++;
	if (count_input == 4) {
...
uint8_t Port_SymbolScreen::GetPort() {
	return Port::GetPort(); 
}

std::vector<std::vector<uint8_t>> Port_SymbolScreen::Get_Array_symbols() {
	return arraySymbols;
}
std::vector<std::vector<uint8_t>> Port_SymbolScreen::Get_Array_colors() {
	return arrayColors;
}

void Port_SymbolScreen::Reset() {
	Init();
	count_input = 0;
	index_raw = 0;
	index_col = 0;
...
std::shared_ptr<Momento> Port_SymbolScreen::SaveState() {
...
void Port_SymbolScreen::RestoreState(const std::shared_ptr<Momento>& momento) {
