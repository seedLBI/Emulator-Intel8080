...
void Port_AsyncKeyboard::SetInput(const uint8_t& value) {
	switch (count_input)
	{
	case 0:
		currentKey = value;
...
uint8_t Port_AsyncKeyboard::GetOutput() {
	if (count_input == 1) {
...
uint8_t Port_AsyncKeyboard::GetPort() {
	return Port::GetPort();
}

void Port_AsyncKeyboard::Reset() {
	currentKey = -1;
	count_input = 0;
}


std::shared_ptr<Momento> Port_AsyncKeyboard::SaveState() {
	return std::shared_ptr<Momento>(new AsyncKeyboard_Momento(count_input,currentKey) );
}
void Port_AsyncKeyboard::RestoreState(const std::shared_ptr<Momento>& momento) {
