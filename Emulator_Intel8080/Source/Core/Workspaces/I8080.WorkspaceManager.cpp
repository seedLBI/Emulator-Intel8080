...
nlohmann::json I8080_WorkspaceManager::SaveSetting() {

	nlohmann::json result;

	result["SaveCurrentWorkspaceBeforeClosingApp"] = SaveCurrentWorkspaceBeforeClosingApp;

	return result;
}
void I8080_WorkspaceManager::LoadSetting(const nlohmann::json& Data) {

	if (Data.contains("SaveCurrentWorkspaceBeforeClosingApp"))
		SaveCurrentWorkspaceBeforeClosingApp = Data["SaveCurrentWorkspaceBeforeClosingApp"].get<bool>();

}


nlohmann::json I8080_WorkspaceManager::Save() {
	...
	for (int i = 0; i < workspaces.size(); i++) {
		result["workspaces"][i] = workspaces[i].Save();
	}

	result["index_choosed"] = IndexChoosed;

	return result;
}


void I8080_WorkspaceManager::Load(const nlohmann::json& Data) {
	...
	if (Data.contains("index_choosed")){

		int index = Data["index_choosed"].get<int>();

		if (index < 0) {
			IndexChoosed = 0;
		}
		else if (index > workspaces.size() - 1) {
			IndexChoosed = 0;
		}
		else {
			IndexChoosed = index;
