...
void Widget_TableInstruction::Draw() {
	...
	if (ImGui::Begin(GetName_c_str(), GetPtrFlagShow())) {
		...
		ImGui::BeginTable("TableOfInstruction",17, flagsTable); {
			...
			ImGui::TableSetupColumn(to_string(0).c_str(), ImGuiTableColumnFlags_WidthFixed, ImGui::GetFontSize() * 2.f);

			for (int i = 1; i < 17; i++){
				ImGui::TableSetupColumn(to_string(i).c_str(), ImGuiTableColumnFlags_WidthStretch);
			}
			...
			for (int i = 0; i < 16; i++)
			{
				...
				string value = int2stringHex(i).substr(1, 1);
				ToUpperALL(value);
			...
			for (int row = 1; row < 17; row++)
			{
				...
				string value = int2stringHex(row - 1).substr(1, 1);
				ToUpperALL(value);
...
std::string Widget_TableInstruction::Save() {
	std::string output = "";
	output += MakeBegin(2);
...
void Widget_TableInstruction::Load(const std::string& Data) {
