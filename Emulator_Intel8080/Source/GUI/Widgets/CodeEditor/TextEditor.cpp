...
TextEditor::TextEditor(FPS_Timer* fps_timer)
		...
		if (ImGui::IsMouseDragging(0) && ImGui::IsMouseDown(0) && IsDragSelection)
		{
			...
			if (Delta_Vertical_Down > 0) {
				...
				if (NeedChangeSymbol >= 0.2f) {
					int countAdd = (int)(NeedChangeSymbol / 0.2f);
					if (countAdd > 3)
						countAdd = 3;
				...
				if (NeedChangeSymbol >= 0.2f) {
					int countSub = (int)(NeedChangeSymbol / 0.2f);
					if (countSub > 3)
						countSub = 3;
...
void TextEditor::DrawSetting() {
	...
	ImGui::SameLine();
	MyHelpMarker(u8"Сохраняет отступы до начала текста при создании новой строки.\nТакже создаёт дополнительный отступ если в строчке присутствует маркер");
	...
	ImGui::SameLine();
	MyHelpMarker(u8"При всплывающей подсказке в пункте Псевдокод вместо\nкатегорий аргументов, будут соответсвующие аргументы из строки.");

}
nlohmann::json TextEditor::SaveSetting() {
	nlohmann::json result;

	result["flag_EnableAutoTab"] = Flag_EnableAutoTab;
	result["flag_AnalyzeLineForInstruction"] = Flag_AnalyzeLineForInstruction;

	return result;
}

void TextEditor::LoadSetting(const nlohmann::json& Data) {

	if (Data.contains("flag_EnableAutoTab"))
		Flag_EnableAutoTab = Data["flag_EnableAutoTab"].get<bool>();

	if (Data.contains("flag_AnalyzeLineForInstruction"))
		Flag_AnalyzeLineForInstruction = Data["flag_AnalyzeLineForInstruction"].get<bool>();
