...
public:
	...
	TextEditor(FPS_Timer* fps_timer);
	~TextEditor();
...
private:
	...
	struct DataFinder {
		...
		void Next() {
			...
			index_current++;
			if (index_current > (int)FindedWords.size() - 1)
				index_current = 0;
		...
		void Prev() {
			...
			index_current--;
			if (index_current < 0)
				index_current = (int)FindedWords.size() - 1;

			FindedWords[index_current].Attention_Time = TimeAttention_Default;
		}


		void Update(FPS_Timer* fps_timer) {
			for (int i = 0; i < (int)FindedWords.size(); i++){
				FindedWords[i].Attention_Time -= (float)fps_timer->GetDeltaTime();
				...
				for (int j = 0; j < utf8Line_text[i].size(); j++) {
					...
					if (utf8Line_text[i][j] == utf8_word[0]) {
						...
						if (AllCorrect) {
							...
							pos_word.Find_Start = Coordinates(index_line, len - (int)utf8_word.size());
	...
	void SetCursorAtCoord_ChoosedWord() {
		...
		float lineee = (float)(_DataFinder.FindedWords[_DataFinder.index_current].Find_Start.mLine - CountLines_In_Window / 2);
