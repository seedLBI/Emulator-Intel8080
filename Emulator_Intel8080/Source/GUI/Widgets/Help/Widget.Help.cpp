...
nlohmann::json Widget_Help::Save() {

	nlohmann::json result = SaveDefaultParameters();

	result["flag_EnableWrapedText"] = EnableWrapedText;

	return result;
}
void Widget_Help::Load(const nlohmann::json& Data) {

	ReadDefaultParameters(Data);

	if (Data.contains("flag_EnableWrapedText"))
		EnableWrapedText = Data["flag_EnableWrapedText"].get<bool>();
...
void Widget_Help::ReadFromMemory() {

	std::stringstream input(str_data_help);
