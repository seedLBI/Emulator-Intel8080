	...
	if (ImGui::Begin(GetName_c_str(), GetPtrFlagShow(), ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_AlwaysAutoResize))
	{
		...
		if (ImGui::BeginPopup("PopupSettingColorPicker"))
		{
			...
			for (int i = 0; i < 48; i++)
			{
				...
				std::string bin_r = to_bin_string((int)FamiliarColor[i].r / 85).substr(6);
				std::string bin_g = to_bin_string((int)FamiliarColor[i].g / 32).substr(5);
				std::string bin_b = to_bin_string((int)FamiliarColor[i].b / 32).substr(5);

				std::string bin_value = "0b" + bin_r + bin_g + bin_b;

				int value = (int)string_formatted_to_value(bin_value);


				if (ImGui::ColorButton(std::string("ButtonFamiliarColor " + std::to_string(i)).c_str(), ImVec4(float(FamiliarColor[i].r / 255.0), float(FamiliarColor[i].g / 255.0), float(FamiliarColor[i].b / 255.0), 1),
				...
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNone)) {
					...
					ImGui::Text(("Hex: " + std::string("0x") + to_hex_string(uint8_t(value))).c_str());
		...
		for (unsigned int h = 0; h < 16; h++) {
			for (unsigned int j = 0; j < 16; j++) {
				uint8_t value = uint8_t(16) * h + j;
				std::string bits = to_bin_string(value);
				uint8_t r = (value >> 6);
				r *= 85;
				uint8_t g = (value << 2);
				g = g >> 5;
				...
				if (ImGui::ColorButton(std::string("ButtonColor " + std::to_string(16 * h + j)).c_str(), ImVec4(float(r / 255.0), float(g / 255.0), float(b / 255.0), 1),
					ImGuiColorEditFlags_NoSidePreview |
					ImGuiColorEditFlags_NoDragDrop |
					ImGuiColorEditFlags_NoSmallPreview |
					ImGuiColorEditFlags_NoAlpha |
				...
				if (!ShowTooltip && ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNone)) {
					...
					ImGui::Text(("Hex: " + std::string("0x") + to_hex_string(value)).c_str());
					ImGui::Text(("Dec: " + std::to_string(value)).c_str());
...
nlohmann::json Widget_ColorPicker::Save() {
	return SaveDefaultParameters();
}

void Widget_ColorPicker::Load(const nlohmann::json& Data) {
