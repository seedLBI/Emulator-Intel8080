#include "Widget.PixelScreenTwoBuffers.h"


Widget_PixelScreenTwoBuffers::Widget_PixelScreenTwoBuffers(GLFWwindow* window,I8080* processor) :I8080_Widget(u8"Продвинутый пиксельный экран") {
	this->window = window;
	this->processor = processor;
	pbo = new PBO(window);
	bufferPixelScreen = new unsigned char[256 * 256 * 4];
}
Widget_PixelScreenTwoBuffers::~Widget_PixelScreenTwoBuffers() {

}
void Widget_PixelScreenTwoBuffers::Draw() {
...
nlohmann::json Widget_PixelScreenTwoBuffers::Save() {

	nlohmann::json result = SaveDefaultParameters();

	result["DrawFrontBuffer"] = DrawFrontBuffer;
...
void Widget_PixelScreenTwoBuffers::Load(const nlohmann::json& Data) {

	ReadDefaultParameters(Data);

	if (Data.contains("DrawFrontBuffer"))
		DrawFrontBuffer = Data["DrawFrontBuffer"].get<int>();
...
void Widget_PixelScreenTwoBuffers::Update() {
	UpdateActive();
	...
	Port_PixelScreenTwoBuffers* p = dynamic_cast<Port_PixelScreenTwoBuffers*>(processor->Get_External_Peripherals()[0x06]);

	std::vector<std::vector<unsigned char>> array_colors;
